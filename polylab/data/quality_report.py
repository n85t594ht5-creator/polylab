...
def build() -> tuple:
    ...
    if lat and lat.get("status") == "OK":
        ...
        findings.append({
            "area": "Изменчивость стакана", "status": "SUPPORTED",
            "detail": f"стакан менялся в {lat.get('book_change_rate', 0)*100:.0f}% интервалов "
                      f"({lat.get('book_changes')} из {lat['samples']})",
            "note": "наблюдение частоты изменений, не причинность", "source": "REAL"})
    else:
    ...
    if n < 500:
        lines.append(f"- Данных пока мало ({n} снимков). Для вывода о качестве серии нужно "
                     ...
                     "а не CONFIRMED.")
    if lat and lat.get("verdict") == "INCONCLUSIVE":
        lines.append("- Задержка при доступной точности источников **не измеряется**. "
                     "Наблюдаемую разницу времён нельзя использовать как торговый сигнал.")
